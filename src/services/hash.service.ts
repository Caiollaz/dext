...
async function digestMessage(message: string, algorithm: string): Promise<string> {
...
export async function generateHashes(input: string): Promise<HashResult[]> {
  ...
  const [sha1, sha256, sha512] = await Promise.all([
    digestMessage(input, 'SHA-1'),
    digestMessage(input, 'SHA-256'),
    digestMessage(input, 'SHA-512'),
  ]);
